...
namespace TicTacToe
{
    ...
    {
        static SerializedPropertyTools()
        {
            cached_properties_field_info = new();
        }
        ...
        private static readonly Dictionary<int, FieldInfo> cached_properties_field_info;

        private static void CreateGetInfoAndStaticTypeDelegate()
        {
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                foreach (var type in assembly.GetTypes())
                {
                    if (type.Name == target_type_name)
                    {
                        ...
                        if (methodInfo != null)
                            _getFieldInfoAndStaticType =
                                (FieldInfoAndStaticType)Delegate.CreateDelegate(typeof(FieldInfoAndStaticType),
                                    methodInfo);
                        else
                ...
                if (_getFieldInfoAndStaticType != null) break;
            }
        }

        private static int GetPropertyHashcode(SerializedProperty property)
        {
            
            var propertyPathIndexNeutralized = property.propertyPath;
            int start = -1;
            while ((start = propertyPathIndexNeutralized.IndexOf(".Array.", StringComparison.Ordinal)) != -1)
            {
                var end = propertyPathIndexNeutralized.IndexOf("]", StringComparison.Ordinal);
        ...
        {
            var key = GetPropertyHashcode(property);
            
            if (cached_properties_field_info.TryGetValue(key, out var fieldInfo))
            {
                return fieldInfo;
            }
            if (_getFieldInfoAndStaticType is null) 
                CreateGetInfoAndStaticTypeDelegate();
            if (_getFieldInfoAndStaticType is not null)
            {
                fieldInfo = _getFieldInfoAndStaticType(property, out _);
                cached_properties_field_info[key] = fieldInfo;
                return fieldInfo;
            }
        ...
        public static List<string> GetStringListFromProperty(SerializedProperty property)
        {
            ...
            {
                keys.Add(property.GetArrayElementAtIndex(i).stringValue);
            }
            return keys;
        }
        ...
        private static bool PropertyTypeStringEquals(string propertyType, Type type)
        {
            ...
            var strippedName = sb.Replace("PPtr<$", "").Replace(">", "").ToString();
